...
class TaskSheetManager {
    ...
    /**
     * Displays a modal dialog for the user.
     *
     * @param {string} htmlFileName - The name of the HTML file to be used for the modal.
     * @param {string} title - The title of the modal dialog.
     * @param {string} action - The intended action (e.g., 'create', 'modify').
     * @param {Array} [currentEditors=[]] - An array of emails of the current editors (optional).
    ...
    static getSheetNameInput() {
        let sheetName = Browser.inputBox("Enter the sheet name (e.g., 業務X: タスクX) *INCLUDE ' : '");
        return sheetName;
    }
    ...
    createNewSheet(checkedStaff) {
        ...
        try {
          ...
          headerRange.setValues([['項目', '概要', '日付', '担当', '完了チェック']])
                      .setBackground("#D3D3D3")
                      .setFontWeight("bold")
                      .setHorizontalAlignment("center")

          // Set hyperlink, background, fontWeight, and horizontalAlignment for "目次に戻る" in A1
    ...
    modifyEditors(checkedStaff, sheetName) {
      try {
        ...
        // Set dropdown for "担当" column based on previously found number of rows
        let dropdownRule = SpreadsheetApp.newDataValidation().requireValueInList(options, true).build();
...
function createNewSheetModal() {
    ...
    manager.showModal('show-editor-choice', 'Choose 担当スタッフ as editors', 'create');
}
...
// Modify editors using the TaskSheetManager class
